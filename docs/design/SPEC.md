...
**A setting that names a column is picked, never typed.** `total_field="sale_total"` is a string to the type system and a choice to a person; derived from `str` it is a text box, whatever is typed validates, and the aggregate then raises on the page. An annotation cannot carry this — the columns are unknown until a block names a DataSource — so the field asks for the mechanism by name, `json_schema_extra={"widget": "column"}`, and core supplies one that knows which columns this viewer may see. `row_link_field` is core's own.

**And which kinds it may name.** `"kinds": ["number"]` narrows the picker, because a setting that will be summed offered a title returns **zero** — a wrong answer with nothing wrong on the screen, which is worse than a failure. A computed column resolves to no model field, so its `sorter` decides its kind, which is the work that hint still does there (§6.1b).

**`columns` belongs to the components that have columns.** It sat on `ComponentConfig`, so every component inherited one — and once the settings form derived itself from the schema, a KPI card offered a column chooser for a setting that meant nothing to it. A KPI reads one aggregate and a gauge one number: they name the field they want and have nothing to choose between. `ColumnsConfig` carries it, the tabular components and the form inherit it, and the chooser is registered there — so the widget follows the **setting**, not the component.
...
**A form draws what the viewer may see, and offers what they may change.** Both, because a form is how a record is read as well as written — drawn from the writable columns alone, somebody holding `view` and not `change` gets an empty card and no way to tell it from a record with nothing in it. That is the whole of "view mode": there is no second component and no flag, only two permissions.

A field shown and not offered carries its **formatted** value — `Yes`, `£8.75`, `bob, cal` — since nothing is going to edit it and that is what a person reads, and carries **no `data-kind`**, which is what the client collects by. So a field the save would refuse cannot be submitted whatever a layout does with it. A form with nothing editable draws no submit button.

A control draws nothing when the viewer may not *see* that column, so one layout serves every viewer. An unregistered layout stacks rather than breaking — the app that registered it may have been uninstalled — and `plinta.components.W001` reports it at boot, because rendering will not.
...
**Moving is not re-pointing.** The positions write touches nothing else — not the block a card draws, not its title, not its tab. Placing a block and arranging it are separate forms for the same reason.

**A new placement lands full width beneath what is there.** A block dropped on top of another is a page somebody has to repair before they can read it.
